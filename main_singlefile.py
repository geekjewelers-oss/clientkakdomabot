...
YANDEX_VISION_ENDPOINT = os.getenv("YANDEX_VISION_ENDPOINT", "https://vision.api.cloud.yandex.net/vision/v1/batchAnalyze")
YANDEX_VISION_ENABLED = bool(
    YANDEX_VISION_API_KEY and YANDEX_VISION_FOLDER_ID
)
YANDEX_VISION_RETRY = _int_env("YANDEX_VISION_RETRY", 2)
...
def _sleep_backoff(attempt: int):
    ...
    base = float(YANDEX_VISION_BACKOFF_BASE)
...
def yandex_vision_extract_text(image_bytes: bytes) -> str:
    ...
    """
    if not YANDEX_VISION_ENABLED:
        _logger.info("[OCR][Vision] disabled (no creds)")
        return ""
    api_key = YANDEX_VISION_API_KEY
    folder = YANDEX_VISION_FOLDER_ID
    endpoint = YANDEX_VISION_ENDPOINT
    if not endpoint:
    ...
    retry = int(YANDEX_VISION_RETRY)
    timeout = float(YANDEX_VISION_TIMEOUT)
...
def ocr_pipeline_extract(img_bytes: bytes) -> dict[str, Any]:
    ...
    min_len = OCR_MIN_EASYOCR_LEN
    skip_vision_if_len = OCR_SKIP_VISION_IF_LEN
    ...
    preferred_source = "easyocr" if preferred == easy_text and easy_text else "tesseract"

    combined_len = len((easy_text or "") + (text_from_tesseract or ""))
    if combined_len >= skip_vision_if_len:
        logger.info("[OCR] Skipping Vision (combined_len=%d >= %d) -> source=%s", combined_len, skip_vision_if_len, preferred_source)
        return {
            "text": preferred,
    ...
    if YANDEX_VISION_ENABLED:
        ...
        logger.info("Vision disabled — skipping")
